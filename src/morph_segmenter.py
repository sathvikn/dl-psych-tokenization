...
def tokenize_sentence(model: transducer, vocab: vocabulary, sentence: str):
    dataloader = prep_data(sentence, vocab, batch_size = 5)
    results = predict(model, dataloader)
    tokenized = " ".join([insert_wb_char(token.split("\t")[1]) for token in results])
    return tokenized
    
def tokenize_corpus(model_dir: str, sentence_list: List[str]):
    sentence_list = sentence_list
    model, vocab = load_model_and_vocab(model_dir)
    dataloader = prep_data(sentence_list, vocab, batch_size = 1000)
    results = predict(model, dataloader)
    return output_as_sentences(results, sentence_list)
...
def output_as_sentences(decoder_output: List[str], corpus_sentences: List[str]):
    token_sentences = []
    start_index = 0
    ...
    for i in tqdm(range(len(words_per_sentence))):
        ...
        processed_decoder_output = [insert_wb_char(token.split("\t")[1]) for token in decoder_output[start_index:end_index]]
